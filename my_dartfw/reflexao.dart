...
String gerar_form(dynamic campos) {
  ...
  InstanceMirror instanceMirror =
      reflect(campos); // cria um InstanceMirror do objeto campos
  ClassMirror classMirror =
      instanceMirror.type; // cria um ClassMirror da classe do objeto campos
  ...
  for (var fieldName in classMirror.instanceMembers.keys) {
    // itera sobre as chaves dos membros de instância da classe
    var field = classMirror.declarations[
        fieldName]; // recupera a declaração do membro de instância
    if (field is VariableMirror) {
      // se o membro for uma variável
      ...
      if (fieldType == Texto) {
        // se o tipo da variável for Texto
        var texto = instanceMirror.getField(field.simpleName).reflectee
            as Texto; // recupera o valor do campo Texto
        html +=
            '<label for="${texto.nome}">${texto.verboso}:</label><br><input type="text" name="${texto.nome}" id="${texto.id}" /> <br><br>'; // adiciona uma label e um input ao html com as propriedades do campo Texto
      } else if (fieldType == Data) {
        // se o tipo da variável for Data
        var data = instanceMirror.getField(field.simpleName).reflectee
            as Data; // recupera o valor do campo Data
        html +=
            '<label for="${data.nome}">${data.verboso}:</label><br><input type="date" name="${data.nome}" id="${data.id}" /> <br><br>'; // adiciona uma label e um input ao html com as propriedades do campo Data
      } else if (fieldType == Frase) {
        // se o tipo da variável for Frase
        var frase = instanceMirror.getField(field.simpleName).reflectee
            as Frase; // recupera o valor do campo Frase
        html +=
            '<label for="${frase.nome}">${frase.verboso}:</label><br><textarea name="${frase.nome}" id="${frase.id}"></textarea> <br><br>'; // adiciona uma label e uma textarea ao html com as propriedades do campo Frase
      }
    }
  }

  html +=
      '<button type="submit">Enviar</button> </form></body></html>'; // adiciona um botão Enviar ao html e finaliza a string html

  File('formulario.html').writeAsString(
      html); // escreve o conteúdo do formulário no arquivo formulario.html

  // Aqui criaria outro formulário
  // File('$className.html').writeAsString(
  //     html); // escreve o conteúdo do formulário no arquivo formulario.html
...
String gerar_tabela(dynamic campos) {
  ...
  // Remove a última vírgula da string da tabela gerada, se houver
